...
/**
 * Check if any word from a list exists in text as a whole word (not substring)
 * For short English words (2-3 chars), uses word boundary regex
 * For longer words or Arabic, uses includes()
...
function containsWord(text, words) {
  ...
  return words.some(word => {
    ...
    // For short English words (2-3 chars), use word boundary regex to avoid false matches
    // e.g., "ai" should not match "raises", "us" should not match "business"
    if (wordLower.length <= 3 && /^[a-z]+$/i.test(wordLower)) {
      const regex = new RegExp(`\\b${wordLower}\\b`, 'i');
      return regex.test(textLower);
    }
    // For longer words or Arabic text, use includes
    return textLower.includes(wordLower);
  });
}

/**
 * Count how many words from a list exist in text as whole words
 * @param {string} text - The text to search in  
 * @param {string[]} words - Words to search for
  ...
  const patternDetectors = [
    {
      ...
      detect: (title) => {
        ...
        const powerCount = countWords(title, powers);
        const hasTension = containsWord(title, tension);
    ...
    },
    {
      ...
      detect: (title) => {
        ...
        return countWords(title, personal) >= 2;
      }
    },
    {
      id: 'hidden_truth',
      ...
      detect: (title) => {
        ...
        return containsWord(title, secrets);
      }
    },
    {
      id: 'tech_race',
      ...
      detect: (title) => {
        const tech = ['ai', 'الذكاء الاصطناعي', 'chip', 'chips', 'رقاقة', 'semiconductor', 'أشباه الموصلات',
                     'nvidia', 'نفيديا', 'tech', 'تقنية', 'technology', 'تكنولوجيا', 'artificial intelligence'];
        const race = ['race', 'سباق', 'war', 'حرب', 'battle', 'معركة', 'competition', 'منافسة',
                     'dominance', 'هيمنة', 'lead', 'تفوق'];
        const hasTech = containsWord(title, tech);
        const hasRace = containsWord(title, race);
    ...
    },
    {
      ...
      detect: (title) => {
        ...
        return containsWord(title, crisis);
      }
    },
    {
      id: 'resource_control',
      ...
      detect: (title) => {
        ...
        const hasResource = containsWord(title, resources);
        const hasControl = containsWord(title, control);
    ...
    },
    {
      ...
      detect: (title) => {
        ...
        return containsWord(title, money);
      }
    },
    {
      ...
      detect: (title) => {
        ...
        return containsWord(title, future);
      }
    }
  ];
  ...
  const patternDetectors = [
    {
      ...
      detect: (title) => {
        const powers = ['us', 'usa', 'china', 'russia', 'iran', 'أمريكا', 'الصين', 'روسيا', 'إيران'];
        const tension = ['vs', 'war', 'حرب', 'conflict', 'صراع', 'sanctions', 'عقوبات', 'crisis'];
        return containsWord(title, powers) && containsWord(title, tension);
      }
    },
    {
      id: 'competitor_tech',
      ...
      detect: (title) => {
        const tech = ['ai', 'chip', 'chips', 'nvidia', 'tech', 'technology', 'الذكاء الاصطناعي', 'رقاقة', 'تقنية', 'artificial intelligence'];
        return containsWord(title, tech);
      }
    },
    {
