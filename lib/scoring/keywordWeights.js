...
// Deprecation warning removed - functions still work and are used for backwards compatibility
...
export const MIN_MATCH_SCORE = 6;

// Minimum weight for a keyword to be considered "high value"
export const HIGH_VALUE_THRESHOLD = 6;

/**
 * Keyword domains - keywords belong to specific topic domains
 * Used for topic coherence validation
 */
const KEYWORD_DOMAINS = {
  // Geopolitics & Countries
  geopolitics: ['russia', 'روسيا', 'china', 'الصين', 'iran', 'إيران', 'ايران', 'ukraine', 'أوكرانيا', 
                'greenland', 'venezuela', 'فنزويلا', 'nato', 'الناتو', 'military', 'عسكري',
                'invasion', 'غزو', 'war', 'حرب', 'sanctions', 'عقوبات', 'syria', 'سوريا',
                'lebanon', 'لبنان', 'egypt', 'مصر', 'israel', 'إسرائيل', 'palestine', 'فلسطين'],
  
  // Finance & Economy  
  finance: ['credit', 'card', 'bank', 'بنك', 'rate', 'فائدة', 'stock', 'أسهم', 'bond', 'سندات',
            'inflation', 'تضخم', 'dollar', 'دولار', 'economy', 'اقتصاد', 'fed', 'federal reserve',
            'wall street', 'investor', 'مستثمر', 'interest', 'borrower', 'debt', 'دين', 'قرض'],
  
  // Technology
  tech: ['tesla', 'تسلا', 'ai', 'الذكاء الاصطناعي', 'self-driving', 'autonomous', 'robot',
         'chip', 'رقاقة', 'nvidia', 'apple', 'google', 'microsoft', 'software', 'hardware',
         'openai', 'chatgpt', 'gpt'],
  
  // Energy
  energy: ['oil', 'نفط', 'gas', 'غاز', 'opec', 'أوبك', 'barrel', 'برميل', 'pipeline', 'خط أنابيب',
           'lng', 'renewable', 'solar', 'nuclear', 'نووي', 'petroleum', 'بترول', 'crude'],
  
  // Generic (can appear in any topic - lower weight when alone)
  generic: ['us', 'usa', 'america', 'أمريكا', 'trump', 'ترامب', 'biden', 'بايدن', 'president', 'رئيس',
            'new', 'says', 'report', 'announces', 'يعلن', 'said', 'week', 'day', 'month']
};

/**
 * Get domains for a keyword
 * @param {string} keyword - The keyword to classify
 * @returns {string[]} Array of domain names this keyword belongs to
 */
function getKeywordDomains(keyword) {
  if (!keyword || typeof keyword !== 'string') return ['generic'];
  
  const normalized = normalizeArabicText(keyword).toLowerCase().trim();
  const domains = [];
  
  for (const [domain, keywords] of Object.entries(KEYWORD_DOMAINS)) {
    // Check if keyword matches any domain keyword
    const matches = keywords.some(kw => {
      const normalizedKw = normalizeArabicText(kw).toLowerCase();
      return normalized === normalizedKw || 
...
export function getKeywordWeight(keyword) {
  // Deprecation warning removed - function still works and is used for backwards compatibility
...
export function calculateMatchScore(matchedKeywords, excludedNames = []) {
  ...
  if (!matchedKeywords || matchedKeywords.length === 0) {
    return {
      ...
      isValidMatch: false,
      debug: !matchedKeywords ? 'No keywords provided' : 'No matching keywords between signal and video'
    };
  }
  ...
  // Track meaningful keywords (with weight > 0)
  const meaningfulKeywords = [];
  
  for (const keyword of filteredKeywords) {
    const weight = getKeywordWeight(keyword);
    
    // NEW: Skip low-weight keywords entirely (< 6)
    // This replaces the need for manual ALWAYS_EXCLUDE lists
    // Only high-value keywords (weight >= 6) count toward matching
    if (weight < 6) {
      continue; // Don't even consider this keyword
    }
    
    const root = getRootConcept(keyword);
    
    // Track meaningful keywords (non-zero weight, now also >= 6)
  ...
  // Require at least 1 meaningful keyword for a valid match
  // This prevents matches on only generic/stopwords (like "الي", "الى", "في", etc.)
  if (meaningfulKeywords.length === 0) {
    return {
      ...
      hasVeryHighValueConcept: false,
      isValidMatch: false,
      debug: `No meaningful keywords (all generic/stopwords): ${filteredKeywords.slice(0, 5).join(', ')}`
    };
  }
  
  // NEW: Check domain coherence
  const keywordDomains = filteredKeywords.map(kw => getKeywordDomains(kw));
  const allDomains = keywordDomains.flat();
  
  // Count non-generic domains
  const specificDomains = allDomains.filter(d => d !== 'generic');
  const domainCounts = {};
  specificDomains.forEach(d => {
    domainCounts[d] = (domainCounts[d] || 0) + 1;
  });
  
  // Require at least 1 specific domain with 2+ keywords, OR 2+ keywords from same domain
  const hasCoherentDomain = Object.values(domainCounts).some(count => count >= 2) ||
                            specificDomains.length >= 2;
  
  // If all keywords are generic (like "us", "trump"), require more keywords
  const onlyGenericKeywords = specificDomains.length === 0;
  if (onlyGenericKeywords && filteredKeywords.length < 4) {
    return {
      score: 0,
      conceptCount: 0,
      concepts: [],
      ...
      hasVeryHighValueConcept: false,
      isValidMatch: false,
      debug: `Only generic keywords (${filteredKeywords.slice(0, 5).join(', ')}) - need 4+ for match`
    };
  }
  
  // If keywords are from different domains with no overlap, likely false match
  const uniqueDomains = [...new Set(specificDomains)];
  if (uniqueDomains.length >= 2 && !hasCoherentDomain) {
    return {
      score: 0,
      conceptCount: 0,
      concepts: [],
      ...
      hasVeryHighValueConcept: false,
      isValidMatch: false,
      debug: `Cross-domain mismatch: ${uniqueDomains.join(', ')} (keywords: ${filteredKeywords.slice(0, 5).join(', ')})`
    };
  }
 // Topic-specific keywords (indicate actual topic, not generic words)
 const TOPIC_SPECIFIC_KEYWORDS = new Set([

    // Finance

    'credit', 'card', 'bank', 'rate', 'stock', 'bond', 'inflation', 'fed', 'federal reserve',

    'interest', 'borrower', 'debt', 'dollar', 'economy', 'economic', 'recession', 'gdp',

    

    // Tech

    'tesla', 'ai', 'self-driving', 'autonomous', 'chip', 'nvidia', 'openai', 'chatgpt', 'gpt',

    'deepseek', 'huawei', 'semiconductor',

    

    // Energy

    'oil', 'gas', 'opec', 'pipeline', 'lng', 'barrel', 'petroleum', 'crude', 'nuclear', 'uranium',

    

    // Countries (ALL major countries for geopolitics!)

    'iran', 'china', 'russia', 'israel', 'ukraine', 'syria', 'lebanon', 'yemen', 'gaza',

    'palestine', 'saudi', 'turkey', 'egypt', 'iraq', 'venezuela', 'greenland', 'taiwan',

    'pakistan', 'india', 'afghanistan', 'qatar', 'uae', 'emirates', 'jordan', 'libya',

    'north korea', 'south korea', 'japan', 'germany', 'france', 'uk', 'britain', 'canada',

    

    // Events/Actions

    'protest', 'protests', 'regime', 'collapse', 'war', 'invasion', 'sanctions', 

    'tariff', 'tariffs', 'election', 'coup', 'revolution', 'military', 'attack', 

    'strike', 'bombing', 'crisis', 'conflict', 'defense', 'pact', 'treaty', 'deal',

    

    // Organizations

    'nato', 'hamas', 'hezbollah', 'houthis', 'irgc', 'taliban', 'isis', 'imf', 'brics',

    

    // Leaders

    'trump', 'biden', 'putin', 'xi', 'netanyahu', 'khamenei', 'zelensky', 'macron', 'musk',

    

    // Arabic - Countries

    'إيران', 'ايران', 'الصين', 'روسيا', 'إسرائيل', 'أوكرانيا', 'سوريا', 'لبنان', 

    'اليمن', 'غزة', 'فلسطين', 'السعودية', 'تركيا', 'مصر', 'العراق', 'فنزويلا', 

    'باكستان', 'الهند', 'أفغانستان', 'قطر', 'الإمارات',

    

    // Arabic - Events

    'احتجاج', 'احتجاجات', 'مظاهرات', 'نظام', 'انهيار', 'حرب', 'غزو', 'عقوبات',

    'رسوم', 'انتخابات', 'انقلاب', 'ثورة', 'عسكري', 'هجوم', 'أزمة', 'صراع', 'دفاع',

    

    // Arabic - Finance/Economy

    'نفط', 'غاز', 'أسهم', 'بنك', 'اقتصاد', 'تضخم', 'دولار', 'قرض', 'دين', 'ذهب',

    

    // Arabic - Leaders

    'ترامب', 'ترمب', 'بايدن', 'بوتين', 'نتنياهو', 'ماكرون'

  ]);



  // Check if at least one keyword is topic-specific
  const hasTopicSpecificKeyword = filteredKeywords.some(kw => {
    const normalized = normalizeArabicText(kw).toLowerCase();
    return TOPIC_SPECIFIC_KEYWORDS.has(normalized);
  });
  
  // If no topic-specific keywords and fewer than 4 keywords, reject
  if (!hasTopicSpecificKeyword && filteredKeywords.length < 4) {
    return {
      score: 0,
      conceptCount: 0,
      concepts: [],
      ...
      hasVeryHighValueConcept: false,
      isValidMatch: false,
      debug: `No topic-specific keywords (only: ${filteredKeywords.slice(0, 5).join(', ')})`
    };
  }
