...
import { extractKeywords, normalizeArabicText, KEYWORD_TRANSLATIONS } from './multiSignalScoring';
import { calculateMatchScore, getRootConcept } from './keywordWeights';
...
export async function validateCompetitorMatchWithAI(signal, competitorVideo, options = {}) {
  ...
  // Expand signal keywords with translations (Arabic ↔ English)
  const expandedSignalKeywords = new Set();
  for (const kw of signalKeywords) {
    expandedSignalKeywords.add(normalizeArabicText(kw).toLowerCase());
    // Add translations for cross-language matching
    ...
    if (rootConcept && KEYWORD_TRANSLATIONS[rootConcept]) {
      for (const translation of KEYWORD_TRANSLATIONS[rootConcept]) {
        expandedSignalKeywords.add(normalizeArabicText(translation).toLowerCase());
      }
    }
  }

  // Find matching keywords in video (now with cross-language support)
  const matchedKeywords = [];
  const matchedConcepts = new Set(); // Track unique concepts to avoid duplicates
  
  for (const kw of expandedSignalKeywords) {
    if (kw.length < 3) continue;
    
    if (videoTitle.includes(kw) || videoDescription.includes(kw)) {
      const rootConcept = getRootConcept(kw);
      if (!matchedConcepts.has(rootConcept)) {
        matchedConcepts.add(rootConcept);
        matchedKeywords.push(rootConcept || kw); // Use root concept for display
      }
    }
  }
  ...
  // Detect if this is cross-language matching (Arabic signal ↔ English video or vice versa)
  const signalHasArabic = /[\u0600-\u06FF]/.test(signal.title || '');
  const videoHasArabic = /[\u0600-\u06FF]/.test(competitorVideo.title || '');
  const isCrossLanguage = signalHasArabic !== videoHasArabic;
  ...
  if (matchedKeywords.length >= 3 && matchResult.isValidMatch) {
    ...
    needsAIValidation = true;
  } else if (isCrossLanguage && signalKeywords.length >= 2) {
    // Cross-language with no keyword matches - use AI to check
    confidence = 30;
    needsAIValidation = true;
    console.log(`   🌐 Cross-language match detected (${signalHasArabic ? 'AR→EN' : 'EN→AR'}), using AI validation`);
  }

  // ============================================
  // STEP 4: AI validation for uncertain cases (30-75 confidence)
  // ============================================
  let aiResult = null;
  if (needsAIValidation && confidence >= 30 && confidence < 75) {
    try {
...
Signal (news article): "${signal.title}"
Video (YouTube): "${competitorVideo.title}"
${matchedKeywords.length > 0 ? `Shared keywords: ${matchedKeywords.join(', ')}` : 'No keyword matches (different languages)'}

Consider: 
- Are they covering the same event, issue, or story? 
- The signal may be in Arabic and video in English (or vice versa) - compare the MEANING, not the words.
- Not just sharing common words - they must be about the SAME specific story.
