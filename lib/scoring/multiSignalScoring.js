...
export async function calculateIdeaScore(idea, context = {}, excludedNames = []) {
  ...
  // Apply context gate: filter topics that require specific context keywords
  const signalFullText = `${idea.title || ''} ${idea.description || ''}`;
  const dnaMatch = filterByContextGate(dnaMatchRaw, signalFullText, dnaTopics);
 ...
 */
export function getUrgencyTier(scoring, idea, dnaData = null) {
  return determineUrgencyTier(idea, scoring, dnaData);
}
...
function calculateExplicitDemand(idea, audienceInterests) {
  ...
  fromComments.forEach(interest => {
    ...
    if (hasMatch && interest.mentionCount >= 5) {
      ...
      const demandScore = Math.min(25, Math.round(interest.mentionCount / 4));
 ...
 * @returns {Object} Tier object with category for backlog
 */
function determineUrgencyTier(idea, scoring, dnaData = null) {
  ...
  for (const video of competitorVideos) {
    ...
    if (!competitorStats[competitorId]) {
      competitorStats[competitorId] = {
        ...
        type: video.competitor_type || video.competitorType || video.competitors?.competitor_type || video.competitors?.type || 'indirect', // Default to indirect if not specified (valid: 'direct', 'indirect', 'trendsetter')
      };
    }
...
function checkTopicCoherence(matchingKeywords, signalTitle, videoTitle, videoDescription) {
  ...
  const topicDomains = {
    ...
    military: ['defense', 'secretary', 'minister', 'war', 'military', 'troops', 'army', 'دفاع', 'وزير', 'حرب', 'عسكري', 'جيش'],
    economy: ['economy', 'economic', 'gdp', 'inflation', 'unemployment', 'اقتصاد', 'اقتصادي', 'تضخم'],
    energy: ['oil', 'gas', 'energy', 'petroleum', 'نفط', 'غاز', 'طاقة'],
