...
import { detectEvergreen } from './evergreenDetector.js';
// ===========================================
...
async function determineUrgencyTier(idea, scoring, dnaData = null) {
 ...
 // ============================================
  ...
  // Check BEFORE this_week to catch explainers early
  // ============================================
  const evergreenResult = detectEvergreen(idea.title, idea.description || '');
  
  // Evergreen criteria:
  // 1. High confidence evergreen detection (>= 70)
  // 2. NOT breaking news (hoursOld > 6 or no urgency signals)
  // 3. NOT a hot trending story (no recent competitor breakout)
  const isNotUrgent = hoursOld > 6 || !hasRecentCompetitorBreakout;
  const isNotHotTrending = !competitorBreakdown.hasDirectBreakout && !competitorBreakdown.hasTrendsetterSignal;
  
  if (evergreenResult.isEvergreen && 
      evergreenResult.confidence >= 70 && 
      isNotUrgent && 
      isNotHotTrending) {
    ...
    const finalTier = {
      ...
      label: 'Evergreen',
      icon: '🌲',
      reason: evergreenResult.reason || 'Educational content - works anytime',
      color: 'blue',
      urgency: 'low',
      evergreenType: evergreenResult.type,
      evergreenConfidence: evergreenResult.confidence,
      evergreenTriggers: evergreenResult.triggers,
    };
    
    if (isDebugIdea || score >= 40) {
      console.log(`   🌲 TIER: EVERGREEN (${evergreenResult.confidence}% confidence)`);
      console.log(`      Type: ${evergreenResult.type}, Triggers: ${evergreenResult.triggers?.join(', ')}`);
      console.log(`===== END TIER DECISION DEBUG =====\n`);
    }
...
function determineBacklogCategory(idea, scoring) {
    ...
    // ============================================
    // 📚 EVERGREEN - Use dedicated detector
    // ============================================
    const evergreenResult = detectEvergreen(idea.title, description);
    
    if (evergreenResult.isEvergreen && evergreenResult.confidence >= 60) {
      console.log(`   🌲 EVERGREEN detected: "${idea.title?.substring(0, 50)}..." (${evergreenResult.confidence}% confidence)`);
      console.log(`      Type: ${evergreenResult.type}, Triggers: ${evergreenResult.triggers?.join(', ')}`);
      ...
      return {
        ...
        color: 'blue',
        confidence: evergreenResult.confidence,
        triggers: evergreenResult.triggers,
      };
    }
