  ...
  for (const [competitorId, stats] of Object.entries(competitorStats)) {
    ...
    const recentVideos = stats.videos.filter(v => {
      ...
      const PERSON_KEYWORDS = new Set([
        ...
        'xi', 'شي', 'jinping', 'jinping', 'macron', 'ماكرون', 'scholz', 'شولتز'
      ]);
      ...
      if (!hasGenericKeywordFilter && matchingKeywords.length > 0) {
        ...
        if (enableDebugForAll || isDebugIdea) {
          console.log(`     ❌ REJECTED by generic filter: Only generic keywords matched (${matchingKeywords.join(', ')})`);
...
function countCompetitorMatches(signalTitle, competitorVideos, days = 7, excludedNames = []) {
  ...
  const rawKeywords = extractKeywords(signalTitle);
  
  // Expand keywords with translations for cross-language matching
  const keywordsSet = new Set();
  for (const kw of rawKeywords) {
    keywordsSet.add(normalizeArabicText(kw).toLowerCase());
    const rootConcept = getRootConcept(kw);
    if (rootConcept && KEYWORD_TRANSLATIONS[rootConcept]) {
      for (const translation of KEYWORD_TRANSLATIONS[rootConcept]) {
        keywordsSet.add(normalizeArabicText(translation).toLowerCase());
      }
    }
  }
  ...
  if (rawKeywords.length < 3) {
    countDiagnostic.lowKeywordCount = rawKeywords.length;
  }
  ...
  if (isDebugIdea) {
    ...
    console.log('   Expanded keywords (with translations):', keywords.slice(0, 20));
    console.log('   Total competitor videos to check:', competitorVideos.length);
  }
  ...
  for (const video of competitorVideos) {
    ...
    const titleKeywords = extractKeywords(video.title || '');
    const descKeywords = extractKeywords((video.description || '').substring(0, 200));
    const rawVideoKeywords = [...new Set([...titleKeywords, ...descKeywords])];
    
    // Expand video keywords with translations too
    const videoKeywordsSet = new Set();
    for (const vk of rawVideoKeywords) {
      videoKeywordsSet.add(normalizeArabicText(vk).toLowerCase());
      const rootConcept = getRootConcept(vk);
      if (rootConcept && KEYWORD_TRANSLATIONS[rootConcept]) {
        for (const translation of KEYWORD_TRANSLATIONS[rootConcept]) {
          videoKeywordsSet.add(normalizeArabicText(translation).toLowerCase());
        }
      }
    }
    const videoKeywords = Array.from(videoKeywordsSet);
    ...
    const matchingKeywords = keywords.filter(kw => {
      ...
      // Step 1: Check if keyword exists in video (now cross-language)
    ...
    if (!hasGenericKeywordFilter && matchingKeywords.length > 0) {
      ...
      if (isDebugIdea || isDebugSignal) {
        console.log(`   ❌ REJECTED: Only generic keywords matched (${matchingKeywords.join(', ')})`);
        console.log(`      Meaningful: ${meaningfulMatches.length}, Generic: ${genericMatches.length}`);
        console.log(`      Country+Person: ${hasCountryPersonCombo} (country: ${hasCountry}, person: ${hasPerson})`);
      }
