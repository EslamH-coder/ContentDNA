...
export async function getAuthUser(request = null) {
  try {
    ...
    // Method 2: Try getUser first (more reliable)
    ...
    // Method 3: Fallback to getSession
    const { data: { session }, error: sessionError } = await supabase.auth.getSession();
    
    if (session?.user) {
      console.log('🔐 getAuthUser - User found via session:', session.user.email);
      return { user: session.user, supabase, error: null };
    }

    console.log('🔐 getAuthUser - No user found');
    return { user: null, supabase, error: userError || sessionError };
